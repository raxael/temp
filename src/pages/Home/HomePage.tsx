...
const PURCHASE_ACTIONS = [
  { id: 'refresh', label: 'Обновить', action: () => {} },
    ...
    <div className="page">
      <section className="card card-header">
        <div>
          ...
          <p className="list-tile__subtitle">Управляйте подписками, платежами и картами</p>
          <button className="stat-chip" onClick={toggleTheme} aria-label="Переключить тему">
            {theme === 'light' ? '☀️ Светлая' : '🌙 Тёмная'}
          </button>
        </div>
        <div className="header-actions">
          <a href={supportProfile.supportLink} target="_blank" rel="noreferrer" className="btn btn-secondary">
            Поддержка
          </a>
          <Link to="/account" className="profile-pill">
            <img src={supportProfile.avatar} alt="Профиль" />
            <div>
              <p>{supportProfile.userName}</p>
              <span>{supportProfile.role}</span>
            </div>
          </Link>
        </div>
      </section>
      ...
      <section className="card">
        ...
        <div className="quick-actions">
          {QUICK_LINKS.map((link) => (
            <Link key={link.id} to={link.href} className={`btn ${link.variant === 'secondary' ? 'btn-secondary' : 'btn-primary'}`}>
              {link.label}
            </Link>
          ))}
        </div>
